...
import { Bell, ChevronDown, ClipboardList, FileCheck, LogOut, Menu, User, X } from "lucide-react";
    ...
    <div className="border-b px-4 py-2 flex items-center justify-between">
        ...
        <nav className="hidden md:block">
          <ul className="flex items-center gap-4 text-md font-normal">
            {["Find Task", "Leaderboard"].map((value, index) => {
              const link = value === "Find Task" ? "/find-task" : "/leaderboard";
              const isActive = location.pathname === link;
              return (
                <Link key={index} className={`relative cursor-pointer transition-all duration-300 hover:underline ${isActive ? "text-[#18cb96]" : ""}`} to={link}>
                  {value}
                </Link>
              );
            })}
      ...
      <div className="flex items-center gap-5">
        {user ? (
          <>
            {user.role === "client" && (
              <Button onClick={() => navigate("/post-task")} className="hidden md:block">
                Post a Task
              </Button>
            )}
            ...
            <Popover>
              ...
              <PopoverContent className="w-[200px] p-2 shadow-md rounded-md">
                <ul className="text-sm">
                  ...
                  {user?.role === "client" && (
                    <Link className="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-secondary" to="/posted-tasks">
                  ...
                  <li className="flex items-center gap-2 px-3 py-2 border-t cursor-pointer hover:bg-secondary" onClick={logout}>
        ...
        <Popover>
          ...
          <PopoverContent className="md:hidden w-[200px] p-4 shadow-lg border rounded-md">
            <ul className="flex flex-col gap-4 text-md font-normal">
              <Link to="/find-task" onClick={() => setMenuOpen(false)}>Find Task</Link>
              <Link to="/leaderboard" onClick={() => setMenuOpen(false)}>Leaderboard</Link>
              {user && user.role === "client" && (
                <Button onClick={() => navigate("/post-task")} className="w-full">Post a Task</Button>
              )}
            </ul>
          </PopoverContent>
        </Popover>
